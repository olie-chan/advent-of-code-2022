...
const parseRange = (line: string): Range[] => line.split(",").map(range => range.split("-").map(Number) as Range);
...
// a fits into b?
const isInclusive = (a: Range, b: Range): boolean => a[0] >= b[0] && a[1] <= b[1];
...
const listHasInclusiveRange = (list: Range[]): boolean => {
	...
	for (let i = 0; i < list.length; i++) {
		...
		result = list.slice(0, i).concat(list.slice(i + 1)).some(range => isInclusive(list[i], range));
	}
	return result;
};
